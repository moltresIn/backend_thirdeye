...
from django.db.models import Q, F, Value, CharField, DateTimeField, IntegerField
...
class FaceView(generics.ListAPIView):
    serializer_class = TempFaceSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = DynamicPageSizePagination

    def get_queryset(self):
        logger.info('FaceView.get_queryset: Building queryset for TempFace')
        queryset = TempFace.objects.filter(user=self.request.user).order_by('-last_seen')
        
        logger.info(f'Initial queryset count: {queryset.count()}')
        logger.info(f'User: {self.request.user}')

        # Log all TempFace objects for this user
        all_faces = list(queryset.values('id', 'last_seen'))
        logger.info(f'All TempFace objects for user: {all_faces}')
        ...
        if date_str:
            try:
                ...
                # Log all TempFace objects after filter
                filtered_faces = list(queryset.values('id', 'last_seen'))
                logger.info(f'TempFace objects after filter: {filtered_faces}')
    ...
    def list(self, request, *args, **kwargs):
      logger.info('FaceView.list: Received list request')
      try:
          ...
          if page is not None:
              ...
              logger.info(f'FaceView.list: Returning page {current_page} of {total_pages} (page size: {page_size})')
            
              return response
        
          logger.warning('FaceView.list: Pagination not applied, returning all data')
          serializer = self.get_serializer(queryset, many=True, context={'request': request})
          return Response(serializer.data)
      except Exception as e:
          logger.error(f'Error in FaceView.list: {str(e)}', exc_info=True)
          raise


class RenameFaceView(generics.UpdateAPIView):
    queryset = TempFace.objects.all()
    serializer_class = TempFaceSerializer
    permission_classes = [IsAuthenticated]

    @swagger_auto_schema(
        request_body=TempFaceSerializer,
        responses={200: TempFaceSerializer()},
    )
    def get_object(self):
        logger.info(f'RenameFaceView.get_object: Getting TempFace object with pk: {self.kwargs["pk"]}')
        obj = get_object_or_404(TempFace, pk=self.kwargs['pk'], user=self.request.user)
        logger.info(f'RenameFaceView.get_object: Found TempFace object: {obj}')
        return obj
    ...
    def update(self, request, *args, **kwargs):
        ...
        logger.info(f'RenameFaceView.update: Updated TempFace {instance.id} with new name: {instance.name}')
        return Response(serializer.data)
...
class RenameCameraView(APIView):
    ...
    def patch(self, request, camera_type, pk):
        ...
        # Clear the cache for stream URLs
